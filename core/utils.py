...
def draw_bbox(image, bboxes, files, iters, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    files.write('Frame {}'.format(iters))
    files.write('\n')

    out_boxes, out_scores, out_classes, num_boxes = bboxes
    for i in range(num_boxes[0]):
        if (classes[out_classes[0][i]] != "car") and (classes[out_classes[0][i]] != "motorbike") and (classes[out_classes[0][i]] != "bus") and (classes[out_classes[0][i]] != "bus"):
            continue
        ...
        files.write(str(coor[1]) + ',')
        files.write(str(coor[0]) + ',')
        files.write(str(coor[3]) + ',')
        files.write(str(coor[2]) + ',')
        files.write('\n')
        files.flush()
